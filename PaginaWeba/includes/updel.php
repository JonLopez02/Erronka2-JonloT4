...
if(isset($_GET['pic_id'])){
    ...
    $query = "SELECT pic FROM produktuak WHERE id LIKE ?";
    $sentencia => $conexion->prepare($query);
    $sentencia->bindParam(s, $name);
    $sentencia->execute();
    $prod = $sentencia->fetchAll(); 
    $deletequery = "DELETE FROM produktuak WHERE id LIKE ?";
    $sentencia2 => $conexion->prepare($deletequery);
    $sentencia2->bindParam(s, $name);
    $sentencia2->execute();
    echo "<div align=center><h5>Produktua ezabatuta</h5></div><br>";
}
...
if(isset($_GET['upload'])){
    ...
    // mysqli_query($conx,"INSERT INTO produktuak (izena, deskripzioa, salneurria, pic, stock) VALUES ('".$_POST['izena']."', '".$_POST['deskripzioa']."', ".$_POST['salneurria'].", '".$_FILES['upfile']['name']."', ".$_POST['stock'].")");
    // echo "<div align=center><h5>Produktu \"".$_POST['izena']."\" txertatuta</h5></div><br>";
    $query = "INSERT INTO produktuak (izena, deskripzioa, salneurria, pic, stock) VALUES (:izena, :deskripzioa, :salneurria, :pic, :stock)";
    $sentencia => $conexion->prepare($query);
    $sentencia->bindParam(':izena', $name);
    $sentencia->bindParam(':deskripzioa', $name);
    $sentencia->bindParam(':salneurria', $name);
    $sentencia->bindParam(':pic', $name);
    $sentencia->bindParam(':stock', $name);
    $sentencia->execute();
    echo "<div align=center><h5>Produktu \"".$_POST['izena']."\" txertatuta</h5></div><br>";
}
    ...
    <table width=1000 cellpadding=10 cellspacing=10 align=center>
        <tr>
            ...
            <td valign=top align=left>
                ...
                $delquery = "SELECT * FROM produktuak";
                $sentencia => $conexion->prepare($query);
                $sentencia->execute();
                $produktuak = $sentencia->fetchAll();
                // $produktuak = array();
